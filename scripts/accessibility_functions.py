...
def get_ugs_to_pop_ratios(accessibility_dict, 
                          census,
                          weights={"330m": 1,"660m": 0.44,"1000m": 0.03,},
                        ):
    """
    Starting from the accessibility dictionary, this function yields the ugs to population ratios
    Args:
        accessibility_dict (dictionary): dictionary with the park area and the census units 
            reachable from a given park, divided by travel time zone
    ...
    population_col = "pop_tot" # this is the name of my population column in the census data
...
def plot_census_points_with_basemap(
    ...
    census = census.to_crs(epsg=3857) # for baseline compatibility
    if sign == 'over':
